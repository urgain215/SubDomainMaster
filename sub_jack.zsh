...
cat $target_dir/live/live-host.txt | nuclei -c 500 -v -o $recon_dir/$target-nuclei-takeovers.txt

 nuclei_takeovers_count=$(grep -c ".*" $recon_dir/$target-nuclei-takeovers.txt)
 ...
 curl -F document=@"$recon_dir/$target-nuclei-takeovers.txt" $telegram_url
